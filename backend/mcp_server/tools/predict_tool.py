...
with standardized JSON responses and Llama integration.
"""
    ...
    async def predict(self, arguments: Dict[str, Any], session_id: str) -> MCPToolResult:
        ...
        try:
            ...
            for symbol in symbols:
                prediction = self._predict_single_symbol(symbol, horizon, request_id)
            ...
            confidence = 0.0
            if predictions:
                valid_predictions = [p for p in predictions if "confidence" in p]
                if valid_predictions:
                    confidence = sum(p["confidence"] for p in valid_predictions) / len(valid_predictions)
