...
from backend.utils.ml_components.stock_analysis_complete import (
...
class AnalyzeTool:
    """
    MCP Analyze Tool
    Provides deep analysis of stock symbols using LLaMA reasoning
    """
                    ...
                    if not model_files:
                        ...
                        try:
                            ...
                            if not success:
                                ...
                                continue
                            logger.info(f"[{request_id}] Models trained for {horizon}")
                        except Exception as e:
    ...
    async def analyze(self, arguments: Dict[str, Any], session_id: str) -> MCPToolResult:
        ...
        try:
            ...
            symbols = list(set(pred.get("symbol") for pred in predictions if "symbol" in pred))
            ...
            for symbol in symbols:
                try:
                    result = self._analyze_single_symbol(symbol, horizons, request_id)
                    analysis_results.append(result)
                    
                    # Calculate confidence from predictions
                    symbol_predictions = result.get("predictions", [])
                    if symbol_predictions:
                        valid_predictions = [p for p in symbol_predictions if "confidence" in p]
                        if valid_predictions:
                            symbol_confidence = sum(p["confidence"] for p in valid_predictions) / len(valid_predictions)
            ...
            self._log_response(request_id, {"analysis_results": analysis_results}, duration_ms)
