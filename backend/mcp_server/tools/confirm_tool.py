...
from backend.utils.ml_components.stock_analysis_complete import LOGS_DIR
...
class ConfirmTool:
    ...
    async def confirm(self, arguments: Dict[str, Any], session_id: str) -> MCPToolResult:
        ...
        try:
            ...
            for action in actions:
                try:
                    ...
                    self._log_confirmation(
                        symbol=symbol,
                        decision="approve" if decision.upper() in ["BUY", "SELL"] else "reject",
                        session_id=session_id,
                        reason=reason
                    )
            ...
            confidence = total_confidence / len(confirmed_actions) if confirmed_actions else 0.0
