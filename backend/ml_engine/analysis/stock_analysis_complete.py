...
class DQNTradingAgent:
    ...
    def __init__(
        ...
        # Q-Networks
        self.q_network = QNetwork(n_features, self.n_actions, hidden_sizes).to(self.device)
        self.target_network = QNetwork(n_features, self.n_actions, hidden_sizes).to(self.device)
        ...
        logger.info(f"DQNTradingAgent initialized with {n_features} features, {self.n_actions} actions")
    ...
    def _calculate_performance_metrics(self) -> Dict:
        ...
        std_reward = np.std(self.rewards_history[-100:]) if len(self.rewards_history) >= 100 else np.std(self.rewards_history)
        
        # Calculate Sharpe ratio (assuming risk-free rate of 0 for simplicity)
        sharpe_ratio = avg_reward / std_reward if std_reward > 0 else 0
        
        # Calculate win rate (positive rewards)
        win_rate = sum(1 for r in self.rewards_history if r > 0) / len(self.rewards_history)
...
class EnhancedDataIngester:
    ...
    def fetch_live_data(
        ...
        for attempt in range(retry_count):
            try:
                print(f"[INFO] Attempt {attempt + 1}/{retry_count} to fetch {symbol} from Yahoo Finance...")
    ...
    def _parse_nse_bhav_csv(self, df: pd.DataFrame, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            symbol_data = df[(df['SYMBOL'] == symbol.upper()) & (df['SERIES'] == 'EQ')]
    ...
    def fetch_all_data(
        ...
        if include_options:
            ...
            try:
                ...
                if all_data['options']:
                    ...
                    all_data['option_chain'][current_expiry] = ticker.option_chain(current_expiry)
        ...
        if include_news:
            ...
            try:
                ...
                print(f"    [OK] News fetched: {len(all_data.get('news', []))} articles")
            except Exception as e:
    ...
    def load_all_data(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            for key, value in data.items():
                if isinstance(value, dict) and 'price_history' in key:
                    # Convert price history back to DataFrame
                    if isinstance(value, dict) and value:  # Check if not empty
    ...
    def _extract_key_metrics(self, info: Dict) -> Dict:
        ...
        # Valuation metrics
        key_metrics['price_to_sales'] = info.get('priceToSalesTrailing12Months')
...
class FeatureEngineer:
    ...
    def __init__(self, feature_cache_dir: Path = FEATURE_CACHE_DIR):
        ...
        logger.info(f"FeatureEngineer initialized with cache dir: {feature_cache_dir}")
    
    def calculate_all_features(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        """Calculate all technical indicators and features"""
        if df.empty:
            logger.error(f"Cannot calculate features for {symbol}: DataFrame is empty")
            return df
        
        print(f"[FEATURES] Calculating 50+ technical indicators for {symbol}...")
        
        # Ensure we have enough data
        if len(df) < 50:
            logger.warning(f"Insufficient data for {symbol} ({len(df)} rows), some indicators may be NaN")
        ...
        print(f"[FEATURES] [OK] Calculated {len(df_features.columns)} features for {symbol}")
    ...
    def _calculate_momentum_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if stoch is not None:
            df['STOCH_K'] = stoch.iloc[:, 0] if isinstance(stoch, pd.DataFrame) else stoch
            df['STOCH_D'] = stoch.iloc[:, 1] if isinstance(stoch, pd.DataFrame) and stoch.shape[1] > 1 else None
    ...
    def save_features(self, df: pd.DataFrame, symbol: str):
        ...
        try:
            ...
            print(f"  Trend Direction: {latest_features.get('trend_direction', 0):.0f}")
            print("")
            print("MOMENTUM INDICATORS:")
            print(f"  RSI_14: {latest_features.get('RSI_14', 0):.2f}")
            print(f"  MACD: {latest_features.get('MACD', 0):.4f}")
            print(f"  MACD Signal: {latest_features.get('MACD_signal', 0):.4f}")
            print(f"  MACD Histogram: {latest_features.get('MACD_hist', 0):.4f}")
            ...
            print(f"  Volatility (20-day): {latest_features.get('volatility_20', 0):.4f}")
            print("")
            print("VOLUME INDICATORS:")
            print(f"  Volume: {latest_features.get('Volume', 0):,.0f}")
            print(f"  OBV: {latest_features.get('OBV', 0):,.0f}")
            print(f"  Volume Ratio: {latest_features.get('volume_ratio', 0):.2f}")
            print(f"{'='*80}\n")
...
def train_ml_models(symbol: str, horizon: str = "intraday", verbose: bool = True) -> Union[bool, Dict]:
    ...
    try:
        ...
        feature_columns = list(feature_data['current_features'].keys())
        
        # Remove non-numeric and target columns
        exclude_columns = ['Date', 'Open', 'High', 'Low', 'Close', 'Volume', 'Dividends', 'Stock Splits']
        ...
        if len(X) == 0:
            if verbose:
                print(f"  -> [ERROR] No valid training data after preprocessing")
            return False
        ...
        if verbose:
            print(f"    [OK] Random Forest - MSE: {rf_mse:.6f}, MAE: {rf_mae:.6f}, R²: {rf_r2:.4f}")
        ...
        if verbose:
            print(f"    [OK] LightGBM - MSE: {lgb_mse:.6f}, MAE: {lgb_mae:.6f}, R²: {lgb_r2:.4f}")
        ...
        if verbose:
            print(f"    [OK] XGBoost - MSE: {xgb_mse:.6f}, MAE: {xgb_mae:.6f}, R²: {xgb_r2:.4f}")
        ...
        for episode in range(episodes):
            ...
            if verbose and (episode + 1) % 10 == 0:
                ...
                print(f"    Episode {episode + 1}/{episodes} - Avg Reward: {avg_reward:.4f}, Epsilon: {dqn_agent.epsilon:.4f}")
        ...
        if verbose:
            ...
            print(f"  -> Random Forest: MSE={rf_mse:.6f}, MAE={rf_mae:.6f}, R²={rf_r2:.4f}")
            print(f"  -> LightGBM: MSE={lgb_mse:.6f}, MAE={lgb_mae:.6f}, R²={lgb_r2:.4f}")
            print(f"  -> XGBoost: MSE={xgb_mse:.6f}, MAE={xgb_mae:.6f}, R²={xgb_r2:.4f}")
        ...
        logger.error(f"Error training ML models for {symbol}: {e}", exc_info=True)
        if verbose:
    ...
    try:
        ...
        if not model_paths:
            ...
            if not dqn_path.exists():
                if verbose:
                    print(f"  -> [ERROR] No trained models found for {symbol} ({horizon})")
                return None
            else:
                # Load DQN model
                ...
                state_values = [current_features.get(col, 0) for col in feature_columns]
                ...
                if verbose:
                    print(f"  -> [OK] DQN Prediction: {predicted_action} (Confidence: {confidence:.4f})")
        ...
        feature_values = [current_features.get(col, 0) for col in feature_columns]
        ...
        try:
            ...
            if dqn_path.exists():
                ...
                action_returns = {0: 0.02, 1: -0.01, 2: 0.005}  # LONG, SHORT, HOLD
                ...
                if hasattr(dqn_agent, 'rewards_history') and dqn_agent.rewards_history:
                    recent_rewards = dqn_agent.rewards_history[-20:] if len(dqn_agent.rewards_history) >= 20 else dqn_agent.rewards_history
        ...
        if predictions:
            ...
            if total_weight > 0:
                # Normalize weights
                normalized_weights = {model: weights[model] / total_weight for model in available_models}
                
                # Calculate weighted average
                ensemble_pred = sum(predictions[model] * normalized_weights[model] for model in available_models)
            else:
                ensemble_pred = 0
        else:
            ensemble_pred = 0
        ...
        if ensemble_pred > 0.01:  # 1% threshold for LONG
            action = "LONG"
            confidence = min(0.95, 0.7 + (ensemble_pred * 10))  # Scale confidence based on return
        ...
        if verbose:
            print(f"  -> [OK] Ensemble Prediction: {action} (Confidence: {confidence:.4f})")
            print(f"  -> Current Price: ₹{current_price:.2f}")
            print(f"  -> Predicted Price: ₹{predicted_price:.2f}")
            print(f"  -> Expected Return: {predicted_return * 100:+.2f}%")
        ...
        logger.error(f"Error predicting stock price for {symbol}: {e}", exc_info=True)
        if verbose:
